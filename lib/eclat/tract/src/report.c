/*----------------------------------------------------------------------
  ...
  History : 2008.08.18 item set reporter created in tract.[ch]
            ...
            2016.02.18 write buffer, preformatting for transaction ids.
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
{                               /* --- print a positive integer */
  ...
  if (rep->pfids                /* if preformatted ids available */
  && (tid <= rep->tidmax))      /* and transaction id is in range */
    isr_tidputs(rep, rep->pfids[tid]);
  else {                        /* if formatting is needed */
    i = BS_INT; tid += 1;       /* get end of output buffer */
    do {                        /* digit output loop */
      buf[--i] = (char)((tid % 10) +'0');    /* store next digit */
      tid /= 10;                /* and remove it from the number */
    } while (tid > 0);          /* while there are more digits */
    isr_tidputsn(rep, buf+i, BS_INT-i);
  }                             /* print the digits */
}  /* isr_tidout() */
...
{                               /* --- print a positive integer */
  ...
  isr_tidputsn(rep, buf+i, BS_INT-i);
  return BS_INT-i;              /* print digits and return length */
}  /* isr_occout() */
...
int isr_tidpfmt (ISREPORT *rep, TID n)
{                               /* --- preformat transaction ids */
  int  i, k;                    /* loop variable */
  TID  t, z;                    /* to traverse the preformatted ids */
  char *s;                      /* to traverse the preformatted ids */
  char buf[BS_INT+1];           /* to preformat a transaction id */

  if (rep->pfids) { free(rep->pfids); rep->pfids = NULL; }
  rep->tidmax = n-1;            /* clear existing transaction ids */
  if (n <= 0) return 0;         /* if no preformatting, abort */
  z = n+n;                      /* one digit plus end-of string */
  for (t = 10; (n >= t) && (TID_MAX /10 > t); t *= 10)
    z += n-t+1;                 /* compute preformatted size */
  rep->pfids = (char**)malloc((size_t)n*sizeof(char*)
                             +(size_t)z*sizeof(char));
  if (!rep->pfids) return -1;   /* get buffer for preformated ids */
  s = (char*)(rep->pfids +n);   /* and pointer to first identifier */
  memset(buf, '0', sizeof(buf));
  buf[k = BS_INT] = '\0';       /* fill preformat buffer with zeros */
  for (t = 0; t < n; t++) {     /* traverse the transaction ids */
    for (i = BS_INT; --i >= 0; ) {
      if (buf[i] >= '9') buf[i] = '0';
      else { buf[i] = (char)(buf[i]+1); break; }
    }                           /* advance the number representation */
    if (i < k) k = i;           /* adapt the start of the number */
    rep->pfids[t] = memcpy(s, buf+k, (size_t)(BS_INT-k+1));
    s += BS_INT-k+1;            /* copy the number representation */
...
{                               /* --- output an item set */
  ...
  if      (rep->tidcnt > 0) {   /* if tids are in ascending order */
    ...
    for (k = 0; k < rep->tracnt; k++) {
      ...
      if (k > 0) isr_tidputs(rep, rep->sep);
      isr_tidout(rep, k);       /* print the transaction identifier */
  ...
  for (s = rep->info; *s; ) {   /* traverse the output format */
    ...
    switch (*s++) {             /* evaluate the indicator character */
      ...
      case 'H': supp = sb-sab;
                ...
                n += isr_numout(rep, 100* conf,               k); break;
      case 'j': supp = sa-sab;
                conf = (sa   > 0) ? (double)supp/(double)(body-sb) : 0;
                n += isr_numout(rep,      conf,               k); break;
      case 'J': supp = sa-sab;
                conf = (sa   > 0) ? (double)supp/(double)(body-sb) : 0;
                n += isr_numout(rep, 100* conf,               k); break;
      case 'k': supp = sb-sab;
                conf = (sb   > 0) ? (double)supp/(double)(body-sa) : 0;
                n += isr_numout(rep,      conf,               k); break;
      case 'K': supp = sb-sab;
                conf = (sb   > 0) ? (double)supp/(double)(body-sa) : 0;
                n += isr_numout(rep, 100* conf,               k); break;
